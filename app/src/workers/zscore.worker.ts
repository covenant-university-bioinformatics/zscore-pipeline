...
import { ZscoreJobsModel, JobStatus } from '../jobs/models/zscore.jobs.model';
...
export default async (job: SandboxedJob) => {
  ...
  writeZscoreFile(jobParams.inputFile, filename, {
  ...
  const jobParameters = [pathToInputFile, pathToOutputDir];
  ...
  await ZscoreJobsModel.findByIdAndUpdate(
    job.data.jobId,
    {
  ...
  const jobSpawn = spawnSync(
    ...
    './pipeline_scripts/Zscore_calculation.sh',
    jobParameters,
  ...
  let zscoreFile: boolean;

  let resultsName = parameters.useTest ? 'test.txt' : name;

  zscoreFile = await fileOrPathExists(`${pathToOutputDir}/${resultsName}`);
